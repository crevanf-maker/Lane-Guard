# main.py (patched: bounding-box-line intersection, forward screening line, stable IDs)
...
# change this ratio to move the screening line forward/back
# 0.0 = top of frame, 1.0 = bottom of frame. Default 0.45 is a bit forward of center.
SCREEN_LINE_RATIO = 0.45
...
class Page1(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.time_var.set(self.format_time(self.elapsed_time))
                # timer job handle for after()/after_cancel()
        self.timer_job = None
        ...
        # Use a canvas that fits into your window — original used 1920x1080 which may be too large on some laptops.
        # We keep the original size but you may want to reduce it if your screen is smaller.
        self.videobox = CTkCanvas(background, width=960, height=540, bg="grey10")
        ...
        # enable file selection by default so upload is visible to user
    ...
    def update_time(self):
        ...
        self.time_var.set(self.format_time(self.elapsed_time))
        # reschedule and keep the id so we can cancel reliably
        self.timer_job = self.duration.after(100, self.update_time)
    ...
    def crop_image(self,img, id):
        ...
        # ensure id in filename is an integer (stable)
        try:
            id_for_name = int(id)
        except Exception:
            id_for_name = id

        file_name = f'{id_for_name}_{x.hour}_{x.minute}_{x.second}_{x.microsecond}.png'
    ...
    def start_detection(self):
        ...
        try :
            if self.checkbox_var.get():
                ...
                # if manual input is used, parse it; otherwise use default cam 0
                manual_str = self.manual_input.get(1.0, "end-1c").strip()
                if manual_str == "":
                    cap = cv2.VideoCapture(0)
                else:
                    try:
                        # if numeric, treat as camera index
                        cap_idx = int(manual_str)
                        cap = cv2.VideoCapture(cap_idx)
                    except:
                        cap = cv2.VideoCapture(manual_str)
             
        except Exception as e :
            error_message = f"Video Source Is Empty or cannot open source.\nDetails: {e}"
            CTkMessagebox(self,title="Error",message=error_message, height=200,width=400,icon="warning",
                          font=("Bahnschrift SemiBold SemiConden",14))
            self.on_stop()
            return
        ...
        # ensure cap opened
        if not cap.isOpened():
            CTkMessagebox(self,title="Error",message="Cannot open video source.", height=200,width=400,icon="warning",
                          font=("Bahnschrift SemiBold SemiConden",14))
            self.on_stop()
            return
        ...
        temprature,elevation = curr_loc.temprature_data(lat,long)
        data, counter1 = {}, []
        # coord_y_line1, offset = 700, 40  # old fixed
        offset = 40  # tolerance in pixels for center-based detection
        width, height = {}, {}
        ...
        def update_frame():
            ...
            try:
                ...
                if not ret or frame is None:
                    # gracefully stop detection if stream ends
                    self.stop_detection()
                    return
                ...
                # resize to canvas size so output fits in the GUI
                try:
                ...
                # compute screening line position relative to current frame height
                frame_h = frame.shape[0]
                # ensure ratio bounded
                ratio = max(0.0, min(1.0, SCREEN_LINE_RATIO))
                line_y = int(frame_h * ratio)

                # run model prediction (this is heavy)
                ...
                for index, row in px.iterrows():
                    # row may contain floats; convert carefully
                    try:
                        x1, y1, x2, y2, _, d = map(int, row)
                    except Exception:
                        continue
                    c = class_list[d] if d < len(class_list) else str(d)
                    if 'pothole' in c:
                ...
                for bbox in bbox_id:
                    ...
                    # Condition 1: bounding box intersects screening line
                    bbox_intersects_line = (y3 <= line_y <= y4)

                    # Condition 2: center near line (for small objects)
                    center_near_line = abs(cy - line_y) <= offset

                    if bbox_intersects_line or center_near_line:
                        # draw visible bounding box
                        cv2.rectangle(frame, (x3, y3), (x4, y4), (0, 255, 0), 5)
                        cvzone.putTextRect(frame, f'id: {id} pothole', (x3, y3 - 20), 1, 1)

                        if id not in counter1:
                            # timestamp & approx size
                            day, date, hour = curr_loc.time_stamp()
                            width_val = round((x4 - x3) * 0.1949152542, 2)  # predict width approx
                            height_val = round((y4 - y3) * 0.11392405, 2)  # predict height approx
                            cv2.putText(frame, f'W:{width_val}cm H:{height_val}cm', (x3, y3 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

                            crop = frame[y3:y4, x3:x4] if (y4 > y3 and x4 > x3 and y4 <= frame.shape[0] and x4 <= frame.shape[1]) else None

                            if self.checkbox_var_save.get() and crop is not None:
                                try:
                                    self.crop_image(crop, id)
                                except Exception as e:
                                    print('Failed to crop/save image:', e)

                            counter1.append(id)
                            # stable store, convert id to int/string
                            self.data[int(id) if isinstance(id, (int, float)) else id] = {
                                'Latitude': lat,
                                'Longtitude': long,
                                'Day': day,
                                'Date': date,
                ...
                # draw screening line (coordinates correspond to resized frame)
                cv2.line(frame, (3, line_y), (frame.shape[1]-3, line_y), (0, 225, 225), 5)

                cv2.putText(frame, text=f'Count = {len(counter1)}', org=(30, 450 if frame.shape[0] > 450 else frame.shape[0]-20),
                ...
                cv2.putText(frame, text='Screening Line', org=(10, 600 if frame.shape[0] > 600 else frame.shape[0]-10),
                            fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1,
                            color=(0, 255, 0), thickness=2)

                # convert to PhotoImage and display on CTkCanvas
                rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                self.photo = ImageTk.PhotoImage(image=Image.fromarray(rgb_frame))

                # delete previous images to avoid piling up
                try:
                    self.videobox.delete("all")
                except Exception:
                    pass

                # anchor 'nw' avoids needing tk.NW (string is accepted)
                self.videobox.create_image(0, 0, anchor='nw', image=self.photo)

                # schedule next frame
                self.videobox.after(10, update_frame)

            except Exception as e:
                # catch exceptions so the GUI update loop doesn't die silently
                print("Error in update_frame:", e)
                # schedule next try so UI keeps responsive
                self.videobox.after(200, update_frame)
    ...
    def on_stop(self):
        ...
        if self.timer_job is not None:
            try:
                self.duration.after_cancel(self.timer_job)
            except Exception:
                pass
            self.timer_job = None
    ...
    def stop_detection(self):
        ...
        if self.data:
            ...
            # create dataframe from the dict stored (keys are stable ids)
            df = pd.DataFrame.from_dict(self.data, orient='index')
            df.reset_index(inplace=True)
            df.rename(columns={'index': 'ID'}, inplace=True)

            # ensure ID column is a simple integer/string for consistent lookup
            try:
...
class Page2(ctk.CTkFrame):
    ...
    def map_handler(self):
        ...
        try :
            ...
            # Ensure ID column present and normalized
            if 'ID' not in self.data.columns:
                # try to infer ID from index
                self.data.reset_index(inplace=True)
                self.data.rename(columns={'index': 'ID'}, inplace=True)

            # convert lat/long columns if misnamed variants exist
            # (the repo uses 'Latitude' and 'Longtitude' column names)
    ...
    def location_detail(self):
        # robust lookup: compare ID column as string to entered text so int/string mismatch doesn't break
        try:
            entered = str(self.find_id.get()).strip()
            if entered == "":
                raise ValueError("Empty ID")
            # make sure we have the dataframe loaded
            if not hasattr(self, 'data'):
                raise ValueError("No CSV loaded")

            # compare as strings for robust matching
            mask = self.data['ID'].astype(str) == entered
            if mask.sum() == 0:
                raise ValueError("ID not found")

            selected = self.data[mask].iloc[0]

            id = selected['ID']
            latitude = selected['Latitude']
            longitude = selected['Longtitude']
            ...
            # search matching image files in the folder containing the CSV
            folder_path = str(self.folder_name_csv)
            files_in_folder = os.listdir(folder_path)
            matching_files = [file for file in files_in_folder if self.extract_prefix(file) == str(entered) and file.endswith(".png")]
...
class Page3(ctk.CTkFrame):
    def __init__(self, parent,controller):
        ...
        background = ctk.CTkFrame(self, width=1280, height=720, fg_color="grey20")
        background.place(x=0, y=0)


        self.script_dir = os.path.dirname(os.path.abspath(__file__))
        side_bar = ctk.CTkFrame(background, width=150, height=720, fg_color="gray10", corner_radius=0)
        side_bar.place(x=0, y=0)

        

        # HOME button
        self.home_img = CTkImage(light_image=Image.open(os.path.join(self.script_dir, 'app_asset', 'home.png')), size=(45,45))
        CTkButton(side_bar, text='', fg_color='grey10', hover_color="#fce101",
                  image=self.home_img, height=100, width=75, cursor="hand2",
                  command=lambda: controller.show_frame(StartPage)).place(x=0, y=45)

# CAMERA button
        self.cam_img = CTkImage(light_image=Image.open(os.path.join(self.script_dir, 'app_asset', 'kamera2.png')), size=(45,45))
        CTkButton(side_bar, text='', fg_color='grey10', hover_color="#fce101",
                  image=self.cam_img, height=100, width=75, cursor="hand2",
                  command=lambda: controller.show_frame(Page1)).place(x=0, y=150)

# MAP button
        self.map_img = CTkImage(light_image=Image.open(os.path.join(self.script_dir, 'app_asset', 'map.png')), size=(55,55))
        CTkButton(side_bar, text='', fg_color='grey10', hover_color="#fce101",
                  image=self.map_img, height=100, width=75, cursor="hand2",
                  command=lambda: controller.show_frame(Page2)).place(x=0, y=260)

# DOCUMENT button (current page → highlight with grey20)
        self.doc_img = CTkImage(light_image=Image.open(os.path.join(self.script_dir, 'app_asset', 'document1.png')), size=(55,55))
        CTkButton(side_bar, text='', fg_color='grey20', hover_color="#fce101",
                  image=self.doc_img, height=100, width=75, cursor="hand2",
                  command=lambda: controller.show_frame(Page3)).place(x=0, y=370)
        ...
        file_handler_box = CTkButton(background,width=390,height=230,text='',fg_color="grey10",
                                  font=("Bahnschrift SemiBold SemiConden",16),corner_radius=6,bg_color="grey20",border_color="white",border_width=1,
                                 state="disabled")
        ...
        self.stop_analyze.configure(state="disabled")
                # SAVE PDF BUTTON
        self.save_pdf_button = CTkButton(file_handler_box,
                                         text="SAVE PDF",
        ...
        self.save_pdf_button.place(x=215,y=155)
    ...
    def on_start (self):
        self.save_pdf_button.configure(state="normal", fg_color="#228B22")
    ...
    def on_stop(self):
        ...
        self.save_pdf_button.configure(state="disabled", fg_color="grey50")
    ...
    def save_as_pdf(self):
        if not hasattr(self, "data") or self.data.empty:
            CTkMessagebox(self, title="Error", message="No data available to export", icon="cancel")
            return
        ...
        title = Paragraph("<b>Northeast Highway Corporation<br/>"
                          "DEPARTMENT OF SAFETY AND HIGHWAY MAINTENANCE<br/>"
                          "POTHOLE OBSERVATION REPORT DOCUMENT</b>", styles["Title"])
        elements.append(title)
        elements.append(Spacer(1, 12))

        # Info
        summary_text = f"""
        Total potholes detected: {self.total_rows}<br/>
        Width Average: {round(self.mean_width,1)}<br/>
        Length Average: {round(self.mean_lenght,1)}
        """
        elements.append(Paragraph(summary_text, styles["Normal"]))
        elements.append(Spacer(1, 12))

        # Table
        data_for_table = [self.data.columns.tolist()] + self.data.values.tolist()
        ...
        CTkMessagebox(self, title="Success", message=f"PDF saved to {save_path}", icon="info")
...
if __name__ == "__main__":
    ...
    # safe icon load — don't crash if icon missing or invalid
    try:
        app.iconbitmap(os.path.join(script_dir, 'app_asset', 'hat.ico'))
    except Exception as e:
        print("Warning: could not load app icon:", e)
